...
const bookForm= document.getElementById('bookForm'); //Forumlário de adição
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    renderBooks(); //Exibe livros na tela
});

//Adiciona evento de submit ao formulário
bookForm.addEventListener('submit', function(e){
    e.preventDefault(); //Impede o comportamento padrão do foprmulário

    //Obtém os valores dos campos do foprmulário
    ...
    //Se o livro já existe, mostrea mensagem de erro
    if (existingBook){
        showMessage('Este livro já está na bilioteca!', 'error');
...
function renderBooks() {
    ...
    //Se não há livros, exibe a mensagem
    if (books.length===0){
        bookList.innerHTML='<p>Nenhum livro cadastrado ainda.</p>';
        return; //Para a execução da função
    }

    //Para cada livro no array, cria um elemnto na lista
    books.forEach(book => {
        ...
        <div class="book-info">
            ...
            <div class="book-status ${book.status === 'disponível' ? 'disponível': 'alugado'}">
                Status:${book.status}
            </div>
        </div>
        <div class="actions">
...
function removeBooke(id){
    //Confirma se o usuário realmenter quer remover
    if (confirm('Tem certeza que deseja remover este livro?')){
        //Filtra o array, mantendo apenas livros com ID diferente
        books = books.filter(book => book.id !== id);
        saveBooksToStorage(); //Salva as alterações
        renderBooks(); //Atualiza a exibição
